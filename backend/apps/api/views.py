...
class UserViewSet(
    ...
    def set_password(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            serializer.save()
        return Response(
            {"detail": "Пароль изменен"}, status=status.HTTP_204_NO_CONTENT
        )
...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def shopping_cart(self, request, **kwargs):
        ...
        if request.method == "DELETE":
            get_object_or_404(Shopping_cart, user=request.user, recipe=recipe).delete()
            return Response(
    ...
    def download_shopping_cart(self, request, **kwargs):
        ingredients = (
            Recipe_ingredient.objects.filter(recipe__shopping_recipe__user=request.user)
            .values("ingredient")
